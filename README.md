...
- **Sub-agents.** A built-in `task` tool lets the model spawn a sub-agent: a fresh conversation with the currently active tools (minus `task` itself, so nesting is one layer deep by construction), the same model, and a briefing the model writes in full (`prompt`, optional `systemPrompt`) - the sub-agent sees nothing of the main conversation. Its final response lands as the tool result with its token usage attached, and its output streams into the task row while it runs. Sub-agent tool calls pass through the same permission rules and output bounding as the main agent's. The `maxSubAgents` setting (integer `>= 1`, default 2) caps how many run at once; further spawns queue until a slot frees.

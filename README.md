...
- **Sub-agents.** A built-in `task` tool lets the model spawn a sub-agent: a fresh conversation with the currently active tools (minus `task` itself, so nesting is one layer deep by construction), the same model, and a briefing the model writes in full (`prompt`, optional `systemPrompt`) - the sub-agent sees nothing of the main conversation. Its final response lands as the tool result with its token usage attached. Each sub-agent renders as a `Task N` window that behaves exactly like the reasoning preview (same shared machinery): a live header with a timer over a fixed-height faded tail while it runs, then a frozen-duration header (persisted with the result, so it survives reload) whose collapsed view shows a short tail and whose expanded view (`ctrl+o`) shows the sub-agent's whole conversation - its prompt, thinking, assistant turns, and every tool call with its result - with head+tail excerpts per message so huge outputs stay bounded. Sub-agent tool calls pass through the same permission rules and output bounding as the main agent's. The `maxSubAgents` setting (integer `>= 1`, default 2) caps how many run at once; further spawns queue until a slot frees, and `taskTimeoutMs` (default 10 minutes, 0 disables) ends a hung run so slots do not stay held forever.

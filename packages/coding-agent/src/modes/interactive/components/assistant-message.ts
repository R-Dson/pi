...
import { animatedPreviewDots, createPreviewWindow } from "./preview-window.ts";
...
export class AssistantMessageComponent extends Container {
	...
	/**
	 * Append the shared tail window (see preview-window.ts) for a thinking
	 * run's text: the last PREVIEW_WINDOW_LINES visual lines, reserved at full
	 * height while the run is live, natural height once it ends, faded toward
	 * the terminal background when it reported one.
	 */
	private addThinkingTailBlock(rawText: string, fixedHeight: boolean): void {
		this.contentContainer.addChild(
			createPreviewWindow(rawText, { fixedHeight, outputPad: this.outputPad, fade: true }),
		);
	}
			...
			if (content.type === "text" && content.text.trim()) {
				...
				if (this.hideThinkingBlock) {
					...
					if (this.isStreaming && !runEnded) {
						...
						header = `${labelBase}${animatedPreviewDots(elapsedMs)} ${(elapsedMs / 1000).toFixed(1)}s`;
					} else {
						header =
							this.thinkingDurationMs !== undefined

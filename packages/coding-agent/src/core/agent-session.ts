...
export class AgentSession {
	...
	/**
	 * Runs actions deferred by handlers during the agent_settled emission (a
	 * prompt or trigger-turn message parks in `_deferredSettledActions` instead
	 * of racing the remaining handlers). Called only after the settle window
	 * closes: these actions start new agent runs, and `_runAgentPrompt` waits
	 * out `_settleWait` before starting, so running them inside the emission
	 * would deadlock the park against the window that is supposed to release it.
	 */
	private async _runDeferredSettledActions(): Promise<void> {
	...
	private async _runAgentPrompt(messages: AgentMessage | AgentMessage[]): Promise<void> {
		...
		try {
			...
			await this._runDeferredSettledActions();
		}
	}
		...
		return this._requestObserver.withRequestKind("compaction", () =>
			compact(
				...
				requestModel,
				{
					...
					systemMessage: getInitialSystemMessage(this.agent.state.messages),
				},
				apiKey,
				headers,
				customInstructions,

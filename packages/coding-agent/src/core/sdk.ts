...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	const defaultActiveToolNames: ToolName[] = ["read", "bash", "edit", "write", "task"];
	...
	const convertToLlmWithBlockImages = (messages: AgentMessage[]): Message[] => {
		...
		return replaceImagesWithPlaceholders(converted);
	};
		...
		return {
			...
			transformHeaders: async (requestHeaders) => {
				...
				const attributionHeaders = settingsManager.getProviderAttributionEnabled()
					? openRouterAttributionHeaders(requestModel)
					: undefined;

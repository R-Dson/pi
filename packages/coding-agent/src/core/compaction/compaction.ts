...
import {
	buildReplayMessages,
	buildReplaySummarizationContext,
	buildStandaloneSummarizationContext,
...
export type {
	/** Request prefix a replaying summarizer call must reproduce byte for byte. */
	SummarizationPrefix,
} from "./replay.ts";
...
export function prepareCompaction(
	...
	// since the previous checkpoint, as the canonical projection selected them;
	// summary semantics stay unchanged.
	...
	// Replay messages: what the model actually saw (replay.ts owns the
	// construction) — the checkpoint's own projected messages (system message
	// plus summary) heading the new messages, so the summarizer request's
	// prefix equals the prior request's from message 0.
	const replayMessages = buildReplayMessages(
		prevCompactionIndex >= 0 ? projectedEntries[prevCompactionIndex].messages : undefined,
		messagesToSummarize,
	);
...
async function generateTurnPrefixSummary(
	...
	const response = await completeSummarization(
		model,
		buildStandaloneSummarizationContext(promptText),

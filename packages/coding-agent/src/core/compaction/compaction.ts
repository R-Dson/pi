...
export function prepareCompaction(
	...
	// construction) — the checkpoint projection heading the new messages, so
	// the summarizer request's prefix equals the prior request's from message 0.
	const replayMessages = buildReplayMessages(pathEntries, prevCompactionIndex, messagesToSummarize);

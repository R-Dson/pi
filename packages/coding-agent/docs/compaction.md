...
The split-turn turn-prefix summary is the exception: it summarizes a fragment, not a replayable request, so it serializes messages to text via [`serializeConversation()`](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/src/core/compaction/utils.ts) (also exported for extension use):

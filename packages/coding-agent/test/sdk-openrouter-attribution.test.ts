...
describe("createAgentSession provider attribution headers", () => {
	...
	it("adds no identification headers for OpenRouter models", async () => {
	...
	it("adds no identification headers for custom providers routed through OpenRouter", async () => {
	...
	it("adds no identification headers for NVIDIA NIM endpoints", async () => {
	...
	it("adds no identification headers for the NVIDIA provider", async () => {
		const headers = await captureHeaders(createModel("nvidia", "https://example.test/v1"));

		expect(headers?.["X-BILLING-INVOKE-ORIGIN"]).toBeUndefined();
	});

	it("adds no identification User-Agent for Cloudflare endpoints", async () => {
		const headers = await captureHeaders(
			createModel("cloudflare-workers-ai", "https://api.cloudflare.com/client/v4"),
		);
	...
	it("passes through provider and request headers untouched", async () => {
		const headers = await captureHeaders(createModel("openrouter", "https://openrouter.ai/api/v1"), {
			providerHeaders: {
		...
		expect(headers?.["X-Custom"]).toBe("request-value");
	});
});

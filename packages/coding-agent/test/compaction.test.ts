...
describe.skipIf(!process.env.ANTHROPIC_OAUTH_TOKEN)("LLM summarization", () => {
	const prefix = { systemPrompt: "You are a helpful assistant. Be concise.", tools: [] };
	...
	it("should generate a compaction result for the large session", async () => {
		...
		const compactionResult = await compact(preparation!, model, prefix, process.env.ANTHROPIC_OAUTH_TOKEN!);

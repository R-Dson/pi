...
- Added a live preview of the newest thinking run while thinking is hidden: a header line with a live elapsed timer over a block of the last few visual lines of the run (newlines preserved, wrapped at the render width, ellipsis marking folded-away content), older lines receding to gray with only the newest line at full color; collapsing to a one-line duration marker when the run ends or the message finishes ([#30](https://github.com/R-Dson/pi/issues/30)).

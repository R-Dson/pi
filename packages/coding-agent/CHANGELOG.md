...
- Added the tool permission engine as exported library API plus an example extension: a pure rule evaluator (tool name, capability, path prefix, command prefix; deny > ask > allow precedence; user rules overriding profile presets; `hide: true` deny rules) with profiles `code`, `review`, and `minimal`, consumed by `examples/extensions/permission-policies.ts` (`.pi/permissions.json` policy files, interactive `ask` dialogs) and `examples/extensions/read-only-mode.ts` ([#11](https://github.com/R-Dson/pi/issues/11), [#90](https://github.com/R-Dson/pi/issues/90)).
...
- Added a live preview of the newest thinking run while thinking is hidden: a header line with a live elapsed timer over a block of the last few visual lines of the run (newlines preserved, wrapped at the render width, the fade itself signals content folded away), keeping the tail block under a `Thought for Ns` marker when the run ends or the message finishes (and under the static label for rebuilt messages); tool-interrupted thinking renders headerless on both sides of the call (runs ended by a tool call and the opening runs of continuation messages), so a think-act-think cycle shows one continuous faded stream with no repeated headers ([#30](https://github.com/R-Dson/pi/issues/30)).

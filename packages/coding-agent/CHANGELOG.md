...
- Fixed the fused `thenRun` command bypassing the extension gate: it now fires a `tool_call`/`tool_result` event pair for its bash facet (`toolCallId` `<parent>:thenRun`) through the session's executor, so any extension that gates `bash` — permission policies, confirmation flows, argument rewriters — sees and can block or rewrite fused commands. A blocked command appends `[thenRun:skipped]` with the denial reason and the file change is kept; an extension handler that throws mid-gate maps to the failure marker instead of aborting the edit result. The fused command also inherits the session's bash settings (`commandPrefix`, `shellPath`); previously it always ran in a fresh default shell. The permission-policies extension's `thenRun` special case is deleted.
...
- Added mid-text and chained skill invocation: `/skill:name` tokens now expand anywhere in a prompt, not just at the start, and multiple invocations per prompt are supported. Each token is replaced by the skill's content in place, keeping the surrounding text in its original order; unknown names and tokens not at a whitespace boundary (e.g. `path/to/skill:x`) pass through as literal text. Typing `/` anywhere in the editor opens autocomplete for skill commands, on any line. The interactive TUI and HTML export render each expanded block as its own collapsible `[skill]` entry interleaved with the user-authored text.
...
- Added a privacy step to the first-time setup wizard, and the wizard is no longer gated behind upstream's `PI_EXPERIMENTAL` flag: it runs once per install — on fresh installs, and once more for existing installs that have never answered the privacy questions. The theme question is always first; skipping or cancelling records the privacy decline, so the wizard never asks the same user twice. The theme step offers every registered theme (built-ins plus installed theme resources) with live preview, instead of only Dark/Light, and "Automatic" is always the preselected default: it stores the `/` theme setting (same value the `/settings` theme submenu calls Automatic), which follows the terminal's detected appearance at every startup instead of pinning one theme — confirming without navigating adopts Automatic even if a different theme was set before.
...
- Removed the core `tools.permissions` policy mode (released in `v0.84.3-fork.3`, the only fork release carrying it): core no longer evaluates permission rules or hides tools. Enforcement ships only as the `permission-policies` extension (now built in, see below), configured by `~/.pi/agent/permissions.json` and `.pi/permissions.json` policy files instead of settings and offering interactive `ask` approval. Migrate by moving rules to the policy files; the evaluator, capability vocabulary, and profiles stay exported library API, and settings carrying `tools.permissions` keys load and ignore them. `resolveProfileConfig` now returns the profile's rule list directly instead of a one-field `ProfileConfig` wrapper ([#70](https://github.com/R-Dson/pi/issues/70)).
...
- Added `capability` to `ToolInfo` (the extension API's `getAllTools` view), so extensions can match tools by capability; `examples/extensions/read-only-mode.ts` demonstrates a review pass built on the public seams ([#69](https://github.com/R-Dson/pi/issues/69)).
...
- Added the tool permission engine as exported library API: a pure rule evaluator (tool name, capability, path prefix, command prefix; deny > ask > allow precedence; user rules overriding profile presets; `hide: true` deny rules) with profiles `code`, `review`, and `minimal`, consumed by the built-in `permission-policies` extension (policy files, interactive `ask` dialogs) and `examples/extensions/read-only-mode.ts` ([#11](https://github.com/R-Dson/pi/issues/11), [#90](https://github.com/R-Dson/pi/issues/90)).
- Added the `permission-policies` extension as a hidden built-in: it activates when `~/.pi/agent/permissions.json` exists or, in a trusted project, `.pi/permissions.json` does, with no copy step; with no policy file it does nothing (no visibility change, no call-time decisions). The project file counts only in trusted projects; trust granted mid-session starts call-time enforcement at the next tool call and visibility changes at the next `session_start` or `/reload`. Remove any copied `examples/extensions/permission-policies.ts` install to avoid running the engine twice (double `ask` dialogs); `read-only-mode.ts` remains the minimal copy-me example.
...
- Added counts-first collapsed tool summaries: `read` renders `N lines (ctrl+o to expand)` on success instead of nothing, and `grep` leads with `N matches` (the true count from execute; context lines, notices, and errors excluded) ahead of its match preview ([#100](https://github.com/R-Dson/pi/issues/100), [#102](https://github.com/R-Dson/pi/issues/102)).
- Added an elapsed-seconds line to running tool rows: any tool without a result yet ticks `Elapsed Ns` once per second, so slow tools visibly work; tools that stream partials take over with their own status from the first partial ([#102](https://github.com/R-Dson/pi/issues/102)).
- Added a fade to the thinking preview ([#100](https://github.com/R-Dson/pi/issues/100)): when the terminal reports its background color (OSC 11), the preview's words are colored between the terminal background and the thinking gray, so the oldest visible text dissolves into the background as new reasoning streams in; terminals that do not answer keep the uniform gray preview.
- Added cache-direction relabels to the footer usage stats (`Read/Write/Cache`), matching the `/session` vocabulary ([#99](https://github.com/R-Dson/pi/issues/99)).
- Added seamless thinking flow across tool calls: a thinking run ended by a tool call suppresses its header, and so does the opening run of the continuation message, so a think-act-think cycle renders tails above and below the tool call with zero headers.

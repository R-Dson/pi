...
- Added a live preview of the newest thinking run while thinking is hidden: the last ~120 display columns (what the model is thinking now) with a live elapsed timer while streaming, collapsing to a one-line duration marker when the run ends or the message finishes ([#30](https://github.com/R-Dson/pi/issues/30)).
